...
if echo $TRAVIS_BRANCH | grep "^master-deploy-" > /dev/null; then
  npm run rdp-release # 将 check 中的包复制一份放到 release
...
elif echo $TRAVIS_BRANCH | grep "^listentown-deploy-" > /dev/null; then
  npm run rdp-x64-listentown
  npm run rdp-ia32-listentown
fi;
